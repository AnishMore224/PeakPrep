...
export const CompanyCard: React.FC<CompanyCardProps> = React.memo(({ company }) => {
  ...
  return (
    <div className={`relative flex flex-col md:flex-row items-start md:items-center justify-between p-4 border border-slate-200 rounded-xl bg-white hover:border-indigo-500/50 transition-colors duration-200 w-full`}>
      <div className="flex flex-col md:flex-row items-start md:items-center space-y-2 md:space-y-0 md:space-x-4 w-full md:w-auto">
        ...
        <div className="flex flex-wrap space-x-4 sm:p-0 py-2">
          {company.tags.map((tag, index) => (
            <span key={index} className="text-slate-500 text-sm">{tag}</span>
          ))}
        </div>
        <div className={`absolute top-4 right-5 md:static text-sm font-medium ${company.placed ? 'text-green-600' : 'text-red-600'}`}>
