...
export const joinContest = async (req: Request, res: Response): Promise<any> => {
    try {
        ...
        if (!contestId || !studentId) {
            return res.status(400).json({ message: 'Contest ID and Student ID are required' });
        }
        ...
        if (contest.participants.includes(studentId)) {
            return res.status(400).json({ message: 'Student already joined the contest' });
        }

        contest.participants.push(studentId);
        await contest.save();

        res.status(200).json({ message: 'Student joined the contest successfully', contest });
    } catch (error) {
...
export const createContest = async (req: Request, res: Response): Promise<any> => {
    try {
        const token = req.headers.authorization?.split(' ')[1];
        if (!token) {
            return res.status(401).json({ message: 'Unauthorized' });
        ...
        if (!name || !timeLimit) {
            return res.status(400).json({ message: 'Name and Time Limit are required' });
        }
...
export const updateContest = async (req: Request, res: Response): Promise<any> => {
    try {
        const token = req.headers.authorization?.split(' ')[1];
        if (!token) {
            return res.status(401).json({ message: 'Unauthorized' });
...
export const deleteContest = async (req: Request, res: Response): Promise<any> => {
    try {
        const token = req.headers.authorization?.split(' ')[1];
        if (!token) {
            return res.status(401).json({ message: 'Unauthorized' });
...
export const getContestParticipants = async (req: Request, res: Response): Promise<any> => {
    try {
        const token = req.headers.authorization?.split(' ')[1];
        if (!token) {
            return res.status(401).json({ message: 'Unauthorized' });
...
export const getContestResults = async (req: Request, res: Response): Promise<any> => {
    try {
        const token = req.headers.authorization?.split(' ')[1];
        if (!token) {
            return res.status(401).json({ message: 'Unauthorized' });
